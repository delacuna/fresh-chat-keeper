...
- `[Fresh Live Chat] チャット監視を開始しました` のログが出ない
- `[Fresh Live Chat] チャット要素が見つかりませんでした（タイムアウト）` も出ない
- `[Fresh Live Chat] ページ遷移を検知しました` は `top` コンテキストで出ていた
...
`restoreMessageElement()` が `el.textContent = original` でテキストを書き戻した際、YouTubeの内部処理がこの変更を検知し、親要素（`yt-live-chat-text-message-renderer`）を新しいDOMノードで差し替える。新しいノードには `data-fresh-live-chat-filtered` 属性がないため、MutationObserver が `processMessage()` を呼んで再フィルタしてしまう。
...
`filterCount` 専用のキー `fresh-live-chat_filter_count` を新設し、書き込み主体をキーごとに完全分離した。

| キー | 書き込み主体 | 内容 |
|---|---|---|
| `fresh-live-chat_settings` | ポップアップのみ | displayMode, filterMode, gameId, enabled, progress |
| `fresh-live-chat_filter_count` | Content Script のみ | フィルタカウント |
...
- Cloudflare Workers に `wrangler deploy` でデプロイした本番プロキシ（`https://fresh-live-chat-proxy.playnicelab.workers.dev`）に対して Chrome 拡張から fetch リクエストを送ると `Failed to fetch` エラーが発生した
...
"host_permissions": [
  "*://www.youtube.com/*",
  "https://fresh-live-chat-proxy.playnicelab.workers.dev/*"
]
```
...
## #009 ビルドエラー: Expected ")" but found "className"（JSX の複数要素返却）

**日付:** 2026-04-15

### 起きたこと

`pnpm build` 時に以下のエラーが出た。

```
[vite:esbuild] Transform failed with 1 error:
C:/.../App.tsx:247:9: ERROR: Expected ")" but found "className"

245|          ))}
246|        </select>
247|        <p className="text-xs text-gray-400 mt-1.5">
   |           ^
```

### 原因

JSX では `return (...)` の直下に複数の要素を並列で書くことができない。既存の `<select>` の直後に説明用の `<p>` を追加した際、両要素を包む親要素がなかったため構文エラーになった。
...
React フラグメント `<>...</>` で複数要素を包む。フラグメントは実際の DOM 要素を生成しないため、構造や見た目には影響しない。
...
- JSX の `return` 直下は**単一のルート要素**でなければならない。兄弟要素を追加する場合は `<>...</>` または `<React.Fragment>...</React.Fragment>` で包む
- TypeScript の型チェック（`tsc --noEmit`）ではこのエラーが検出されないことがある。ビルド（esbuild/Vite）で初めて表面化するケースに注意
